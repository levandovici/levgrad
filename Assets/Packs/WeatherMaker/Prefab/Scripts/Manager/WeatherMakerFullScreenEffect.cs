...
namespace DigitalRuby.WeatherMaker
{
    ...
    /// </summary>
    [System.Serializable]
    public partial class WeatherMakerFullScreenEffect : System.IDisposable
    {
        ...
        {
#if UNITY_URP
            if (currentURPColorTarget != null)
            {
                return currentURPColorTarget.nameID;
            }
#endif
            return BuiltinRenderTextureType.CameraTarget;
        }

        public static void SetCameraTarget(CommandBuffer commandBuffer)
        {
#if UNITY_URP
            if (currentURPColorTarget != null && currentURPDepthTarget != null)
            {
                commandBuffer.SetRenderTarget(currentURPColorTarget, currentURPDepthTarget, 0, CubemapFace.Unknown, -1);
                return;
            }
            else if (currentURPColorTarget != null)
            {
                commandBuffer.SetRenderTarget(currentURPColorTarget, 0, CubemapFace.Unknown, -1);
                return;
            }
#endif
            commandBuffer.SetRenderTarget(WeatherMakerFullScreenEffect.CameraTargetIdentifier(), 0, CubemapFace.Unknown, -1);
        }
        ...
        internal static void Blit(CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier dest, Material mat = null, int pass = 0)
        {
            ...
            if (mat == null || cmd == null || pass >= mat.passCount)
            {
                return;
            }

            var resolvedSource = source == 0 && source != BuiltinRenderTextureType.CurrentActive && source != BuiltinRenderTextureType.CameraTarget ? (Texture2D)null : source;
            cmd.Blit(resolvedSource, dest, mat, pass);
        }
...
#if UNITY_URP
                // URP provides the command buffer per pass/action. Do not own or persist one here.
                CommandBuffer = null
#else
        ...
        private void RecordEffectCommands(
            WeatherMakerCommandBuffer weatherMakerCommandBuffer,
            CommandBuffer commandBuffer,
            Camera camera,
            RenderTargetIdentifier cameraTargetIdentifier,
            ...
            System.Action<WeatherMakerCommandBuffer> postSetupCommandBuffer)
        {
            ...
            Material material = weatherMakerCommandBuffer.Material;

            int scale = (int)downsampleScale;
            commandBuffer.SetGlobalFloat(WMS._WeatherMakerDownsampleScale, (float)scale);
            SetDownsampleKeyword(commandBuffer, scale);

            bool usingSharedDepthDownsample = (scale > 1 &&
                WeatherMakerCommandBufferManagerScript.Instance != null &&
                WeatherMakerCommandBufferManagerScript.Instance.IsSharedDepthDownsampleActive(camera));

            // downsample depth if we need it and no shared per-camera depth downsample is active
            if (scale > 1 && !usingSharedDepthDownsample)
            {
                AttachDepthDownsample(commandBuffer, reprojState, DepthDownsampleMaterial, camera);
            }
            else if (usingSharedDepthDownsample && WeatherMakerCommandBufferManagerScript.Instance != null)
            {
                // Shared downsample runs in a separate command buffer/event. Rebind globals so this command buffer sees the right textures.
                WeatherMakerCommandBufferManagerScript.Instance.BindSharedDepthDownsampleGlobals(commandBuffer, camera);
            }
            ...
            if (Enabled && result)
            {
                ...
                if (BlurShaderType == BlurShaderType.None &&
                    ...
                    reprojState == null &&
                    material.passCount < 3 &&
                    postProcessCommandBuffer == null)
                {
                    commandBuffer.SetGlobalFloat(WMS._SrcBlendMode, (float)SourceBlendMode);
                    commandBuffer.SetGlobalFloat(WMS._DstBlendMode, (float)DestBlendMode);
                    WeatherMakerFullScreenEffect.Blit(commandBuffer, cameraTargetIdentifier, cameraTargetIdentifier, material, 0);
                }
                else
                {
                    ...
                    if (reprojState == null || reprojState.TemporalReprojectionMaterial == null)
                    {
                        ...
                        WeatherMakerFullScreenEffect.Blit(commandBuffer, renderTargetRenderedImage, renderTargetRenderedImage, material, 0);
                    }
                    else
                    {
                        AttachTemporalReprojection(commandBuffer, reprojState, renderTargetRenderedImage, material, scale, defaultFormat);
                    }
                    ...
                    {
                        AttachPostProcessing(commandBuffer, material, 0, 0, defaultFormat, renderTargetRenderedImage, reprojState, camera, ref postSourceId, ref postSource, scale);
                    }

                    AttachBlurBlit(commandBuffer, renderTargetRenderedImage, cameraTargetIdentifier, material, blur, defaultFormat, reprojState, camera);
            ...
            System.Action<WeatherMakerCommandBuffer> postSetupCommandBuffer)
        {
            ...
            // URP: register an action that populates the passed-in command buffer on demand
            if (weatherMakerCommandBuffer != null && WeatherMakerCommandBufferManagerScript.Instance != null)
            {
                // remove only our own previous action for this camera/event to prevent accumulation
                previousActions.TryGetValue(camera, out var oldAction);

                // capture all state needed for the action
                var capturedReprojState = reprojState;
                var capturedDownsampleScale = downsampleScale;
                var capturedPreSetup = preSetupCommandBuffer;
                var capturedPostProcess = postProcessCommandBuffer;
                var capturedPostSetup = postSetupCommandBuffer;
                var capturedEffect = this;
                var capturedWmcb = weatherMakerCommandBuffer;

                // store material/reproj on the weathermaker command buffer for UpdateMaterialProperties
                capturedWmcb.Material = clonedMaterial;
                capturedWmcb.ReprojectionState = capturedReprojState;
                capturedWmcb.RenderQueue = RenderQueue;

                // register new action for this camera/event
                System.Action<WeatherMakerCommandBufferContext> newAction = (ctx) =>
                {
                    CommandBuffer commandBuffer = ctx.CommandBuffer;
                    try
                    {
                        // Bind the per-pass URP command buffer only for action execution.
                        capturedWmcb.CommandBuffer = commandBuffer;
                        capturedWmcb.ColorTarget = ctx.ColorTarget;
                        capturedWmcb.DepthTarget = ctx.DepthTarget;

                        // set thread-static context so static CameraTargetIdentifier()/SetCameraTarget() return correct URP targets
                        currentURPColorTarget = ctx.ColorTarget;
                        currentURPDepthTarget = ctx.DepthTarget;

                        capturedEffect.RecordEffectCommands(
                            capturedWmcb,
                            commandBuffer,
                            camera,
                            ctx.CameraTargetIdentifier(),
                            capturedReprojState,
                            capturedDownsampleScale,
                            capturedPreSetup,
                            capturedPostProcess,
                            capturedPostSetup);
                    }
                    ...
                    {
                        // Never persist URP-owned command buffers after pass execution.
                        capturedWmcb.CommandBuffer = null;
                        capturedWmcb.ColorTarget = null;
                        capturedWmcb.DepthTarget = null;
                        currentURPColorTarget = null;
                        currentURPDepthTarget = null;
                    }

                };

                // track this action so we can remove only our own action next frame
                previousActions[camera] = newAction;
                WeatherMakerCommandBufferManagerScript.Instance.ReplaceCameraCommandBufferAction(oldAction, newAction, camera, RenderQueue);
            ...
            // Standard pipeline: pre-build the command buffer as before
            if (weatherMakerCommandBuffer != null && weatherMakerCommandBuffer.CommandBuffer != null)
            {
                if (WeatherMakerCommandBufferManagerScript.Instance != null)
                {
                    WeatherMakerCommandBufferManagerScript.Instance.RemoveCameraCommandBuffer(weatherMakerCommandBuffer.CommandBuffer, camera, RenderQueue);
                }
            ...
            if (WeatherMakerCommandBufferManagerScript.Instance != null)
            {
                // Track once, but always re-attach after rebuilding this frame's command buffer.
                WeatherMakerCommandBufferManagerScript.Instance.AddCommandBuffer(weatherMakerCommandBuffer);
                WeatherMakerCommandBufferManagerScript.Instance.ReplaceCameraCommandBuffer(commandBuffer, camera, RenderQueue);
            }
            return weatherMakerCommandBuffer;
                ...
                for (int i = weatherMakerCommandBuffers.Count - 1; i >= 0; i--)
                {
                    if (weatherMakerCommandBuffers[i].Key == null)
                    {
                        if (weatherMakerCommandBuffers[i].Value.CommandBuffer != null)
                        {
                            ...
                            weatherMakerCommandBuffers[i].Value.CommandBuffer = null;
                        }
        ...
        public void PreCullCamera(Camera camera, bool integratedTemporalReprojection = true)
        {
                ...
                Material != null && clonedMaterial != null && camera != null)
            {
                ...
                if (Enabled)
                {
                    ...
                    if (downsampleScale > WeatherMakerDownsampleScale.FullResolution &&
                        WeatherMakerCommandBufferManagerScript.Instance != null)
                    {
                        WeatherMakerCommandBufferManagerScript.Instance.RequestSharedDepthDownsample(camera);
                    }
                }
            }
        }
        ...
        public void Dispose()
        {
            ...
            {
#if UNITY_URP
                // remove our tracked actions
                foreach (var kvp in previousActions)
                {
                    if (kvp.Key != null)
                    {
                        WeatherMakerCommandBufferManagerScript.Instance.RemoveCameraCommandBufferAction(kvp.Value, kvp.Key, RenderQueue);
                    }
                }
                ...
                foreach (KeyValuePair<Camera, WeatherMakerCommandBuffer> kv in weatherMakerCommandBuffers)
                {
                    ...
                    if (kv.Key != null && kv.Value != null && kv.Value.CommandBuffer != null)
                    {
                        WeatherMakerCommandBufferManagerScript.Instance.RemoveCameraCommandBuffer(kv.Value.CommandBuffer, kv.Key, RenderQueue);
                    }
                }
            }
            ...
            foreach (KeyValuePair<Camera, WeatherMakerCommandBuffer> kv in weatherMakerCommandBuffers)
            {
                if (kv.Value != null && kv.Value.CommandBuffer != null)
                {
                    if (!WeatherMakerRenderBackendUtility.IsURP)
                    {
                        ...
                        {
                            // command buffer may have already been disposed (e.g., URP action-based path)
                        }
                    }
                    kv.Value.CommandBuffer = null;
                }
            }
    ...
    public static class CommandBufferExtensions
    {
        ...
        public static void DrawQuad(this CommandBuffer commandBuffer, RenderTargetIdentifier source, RenderTargetIdentifier dest, Material mat, int pass)
        {
            ...
            commandBuffer.DrawMesh(WeatherMakerFullScreenEffect.Quad, Matrix4x4.identity, mat, 0, pass, null);
        }
    }
}
