...
float auroraLineNoise(inout float4 pos)
{
	...
	//float detailZ = ANTI_ALIAS(pos.z + sin(detailXY.x)); // smooth vertical
    //float detailZ = ANTI_ALIAS(pos.w + detailXY.y); // jagged horizontal
    //float detailZ = ANTI_ALIAS(pos.w + sin(detailXY.y)); // smooth horizontal
