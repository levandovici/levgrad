...
fixed3 SampleAmbientLight(float3 marchPos, float3 rayDir, float rayLength, float skyMultiplier, fixed3 skyColor, float heightFrac, float4 weatherData)
{
	...
	UNITY_BRANCH
	if (coverage > 0.1 && _CloudAmbientShadowVolumetric > 0.0)
	{
		// coverage-weighted AO: thin edges get less darkening, dense cores get appropriate occlusion
		float occlusionWeight = coverage * coverage * _CloudAmbientShadowVolumetric;
		occlusionWeight *= (1.0 + (RandomFloat(marchPos) * 0.05));

		// 2 samples upward for sky occlusion (wider spacing compensates for fewer samples)
		skyReducer -= (occlusionWeight * GetCloudRoundness(marchPos, weatherData, heightFrac + 0.12));
		skyReducer -= (occlusionWeight * GetCloudRoundness(marchPos, weatherData, heightFrac + 0.28));

		// 2 samples downward for ground occlusion
		groundReducer -= (occlusionWeight * GetCloudRoundness(marchPos, weatherData, heightFrac - 0.12));
		groundReducer -= (occlusionWeight * GetCloudRoundness(marchPos, weatherData, heightFrac - 0.28));

		// prevent fully black ambient which looks unnatural
		skyReducer = max(0.05, skyReducer);
		groundReducer = max(0.05, groundReducer);
	}
